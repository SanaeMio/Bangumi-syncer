...
从数据库账号目录构造 BangumiApi 实例，供需要"按当前首选账号临时构造客户端"的场景复用：
...
def build_bangumi_api_from_primary_config(
    ...
    Args:
        section_name: 指定账号段名（用于"我的追番"卡片切换账号）。
            传入时直接从 DB 取该 section 对应账号；None 时取当前首选账号。

    Returns:
        BangumiApi 实例；若无有效配置返回 None。调用方负责 close()。
    """
    # DB 为唯一真相源：section_name 用于精确指定账号，None 走首选账号
    if section_name:
        cfg = get_bangumi_config_by_section(section_name)
    else:
        cfg = get_primary_bangumi_config()
    ...
    if cfg and cfg.get("access_token"):
        try:
            ...
            # 指定 section 时按 section 续期；否则由 auth 服务自行取首选账号
            bangumi_auth_service.refresh_primary_token_if_needed(section_name)
            ...
            logger.warning(
                f"build_bangumi_api: 刷新账号 '{section_name or '首选'}' 的 token 失败: {e}，"
        ...
        if section_name:
            ...
            cfg = get_primary_bangumi_config() or cfg
