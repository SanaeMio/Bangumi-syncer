...
class SyncService:
    ...
    def sync_custom_item(self, item: CustomItem, source: str = "custom") -> SyncResponse:
        ...
        try:
            ...
            if item.media_type != 'episode':
                ...
                return SyncResponse(status="error", message=f"同步类型{item.media_type}不支持")
            ...
            if self._is_title_blocked(item.title, item.ori_title):
                return SyncResponse(status="ignored", message="番剧标题包含屏蔽关键词，跳过同步")
    ...
    def _is_title_blocked(self, title: str, ori_title: str = None) -> bool:
        """检查番剧标题是否包含屏蔽关键词"""
        # 获取屏蔽关键词配置
        blocked_keywords_str = config_manager.get('sync', 'blocked_keywords', fallback='').strip()
        ...
        if config_manager.get('bangumi_data', 'enabled', fallback=True):
            try:
                ...
                if bangumi_data_result:
                    ...
                    if item.season > 1:
                        # 检查匹配到的标题中是否包含季度信息
                        title_has_season_info = self._check_season_info_in_title(matched_title, item.season)
    ...
    def _check_season_info_in_title(self, title: str, season: int) -> bool:
        ...
        # 数字形式
        season_keywords = [
            f'第{season}季', f'第{season}期',
            f'{season}期', f'{season}季',
            f'Season {season}', f'S{season}'
        ]
    ...
    def _get_bangumi_config_for_user(self, user_name: str) -> Optional[Dict[str, str]]:
        ...
        if mode == 'single':
            ...
            return {
                ...
                'access_token': config_manager.get('bangumi', 'access_token', fallback=''),
    ...
    def sync_plex_item(self, plex_data: Dict[str, Any]) -> SyncResponse:
        ...
        try:
            ...
            if plex_data["event"] != 'media.scrobble':
                ...
                return SyncResponse(status="ignored", message=f"事件类型{plex_data['event']}无需同步")
        ...
        try:
            ...
            for field in required_fields:
                if field not in emby_data:
                    ...
                    return SyncResponse(status="error", message=f"请求缺少必要字段: {field}")

            # 检查同步类型是否为看过
            if emby_data["Event"] != 'item.markplayed' and emby_data["Event"] != 'playback.stop':
                logger.debug(f'事件类型{emby_data["Event"]}无需同步，跳过')
                return SyncResponse(status="ignored", message=f"事件类型{emby_data['Event']}无需同步")
            ...
            for field in item_required_fields:
                if field not in emby_data["Item"]:
                    ...
                    return SyncResponse(status="error", message=f"Item缺少必要字段: {field}")
    ...
    def sync_jellyfin_item(self, jellyfin_data: Dict[str, Any]) -> SyncResponse:
        ...
        try:
            ...
            if jellyfin_data["NotificationType"] != 'PlaybackStop':
                ...
                return SyncResponse(status="ignored", message=f"事件类型{jellyfin_data['NotificationType']}无需同步")
