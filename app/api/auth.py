...
async def login(request: Request, response: Response):
    ...
    try:
        ...
        if security_manager.is_ip_locked(client_ip):
            ...
            lockout_str = datetime.datetime.fromtimestamp(lockout_time).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            logger.warning(f"IP {client_ip} 被锁定，拒绝登录请求")
            raise HTTPException(
                status_code=423, detail=f"IP已被锁定，请于 {lockout_str} 后重试"
            )
...
async def auth_status(
    ...
    if current_user:
        ...
        return {"status": "success", "data": {"authenticated": False, "user": None}}
