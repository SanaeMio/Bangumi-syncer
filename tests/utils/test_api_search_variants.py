...
验证 bgm_search 在 archive 未命中、降级到无日期 search 兜底时，
...
def _capture_fallback_search_titles(api: BangumiApi) -> list[str]:
    """记录 bgm_search 兜底阶段 search 被调用的 title 顺序。

    通过 start_date/end_date 是否为空区分兜底搜索与精确搜索：
    - 兜底搜索：start_date="" 且 end_date=""（无日期模式）
    - 精确搜索：带日期参数
...
class TestBgmSearchVariantOrder:
    ...
    def test_simple_title_no_extra_variants(self) -> None:
        ...
        # 无 premiere_date 直接进入 search 兜底
    ...
    def test_dedup_preserve_priority(self) -> None:
...
class TestBgmSearchPreciseSearchPath:
    ...
    def test_precise_search_skips_fallback(self) -> None:
        """精确搜索命中时不进入兜底 search。"""
        api = _make_api()
...
class TestReadApiUnreachableShortCircuit:
    ...
    # search_old 方法已删除（统一走 search），原 test_search_old_skips_request
    # 随之移除。API 不可达短路由 search 的行为由上方 test_search_skips_request 覆盖。
...
class TestBgmSearchNetworkResilience:
    ...
    def test_precise_search_network_error_falls_back_to_fallback(self) -> None:
        """精确搜索网络失败后仍继续兜底变体遍历（不中断）。"""
        api = _make_api()
        seen: list[str] = []
    ...
    # 原 test_get_subject_network_error_skips_candidate 已移除：
    # v0 search 返回完整 Subject（含 infobox），bgm_search 兜底不再逐条
    # 调 get_subject 补全，因此"候选详情拉取网络失败"场景已不存在。
    # 候选相似度过滤（>0.3）由 search 返回数据直接计算，无独立详情请求。
...
class TestNetworkErrorNoAttributeError:
    ...
    # search_old 方法已删除（统一走 search），原
    # test_search_old_http_error_returns_empty_not_attribute_error 随之移除。
    # HTTP 错误返回空结果的行为由上方 test_search_http_error_* 用例覆盖。
