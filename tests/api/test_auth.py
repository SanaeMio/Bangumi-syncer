...
async def test_login_failure_triggers_lockout(mock_security_manager):
    ...
    with patch("app.api.auth.notification_service") as mock_notify:
        async with AsyncClient(
            transport=ASGITransport(app=app), base_url="http://test"
        ) as client:
            response = await client.post(
    ...
    assert mock_notify.notify.call_args.args[0] == "ip_locked"
    assert mock_notify.notify.call_args.kwargs["ip"] == "127.0.0.1"
