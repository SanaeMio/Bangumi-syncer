...
@pytest.mark.asyncio
async def test_get_notification_types_without_summary_jobs(app_with_auth):
    """无 summary 任务时不附加动态类型"""
    with patch("app.api.notification.config_manager") as mock_cm:
        mock_cm.get_summary_configs.return_value = []
        async with AsyncClient(
            transport=ASGITransport(app=app_with_auth), base_url="http://test"
    ...
    assert not any(t.startswith("watching_summary_") for t in ids)
