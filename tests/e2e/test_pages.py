...
def test_dashboard_renders(authed_page):
    """仪表盘页面渲染核心元素"""
    page = authed_page
    # authed_page 已在 /dashboard，直接验证当前页面
...
def test_dashboard_scheduler_status_card(authed_page):
    """仪表盘调度器状态卡渲染（P6 新增）"""
    page = authed_page
    # authed_page 已在 /dashboard
    # 调度器状态卡可能正在异步加载，等待一下
...
def test_config_page_renders(authed_page):
...
def test_config_page_toc_items(authed_page):
    """配置页 TOC 包含核心配置段"""
    page = authed_page
    base = page.url.split("/dashboard")[0]
    page.goto(f"{base}/config")
...
def test_config_page_save_button(authed_page):
    """配置页保存按钮存在且可点击"""
    page = authed_page
    base = page.url.split("/dashboard")[0]
    page.goto(f"{base}/config")
...
def test_notification_rule_modal(authed_page):
    """通知规则编辑模态框可打开"""
    page = authed_page
    base = page.url.split("/dashboard")[0]
    page.goto(f"{base}/config")
