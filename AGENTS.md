...
Bangumi-syncer 将常见媒体库（Plex、Emby、Jellyfin、Trakt、飞牛等）的观看进度同步到 [Bangumi（番组计划）](https://bgm.tv/) 官方 API。提供基于 **FastAPI** 的 Web 管理与同步接口，默认本地访问 `http://localhost:8000`（与 README 一致）。
...
- **包管理**：推荐 [uv](https://docs.astral.sh/uv/)，与 CI 及 CONTRIBUTING 一致。
...
在项目根目录执行（与 [CONTRIBUTING.md](CONTRIBUTING.md) 提交前自检一致）：
...
推送或打开 PR 前建议至少执行上文 **Ruff + pytest**（若改模板则加 **djlint**）。单个 PR 尽量聚焦单一目的，便于审查与回滚（与 CONTRIBUTING 一致）。