...
- 🔌 **常见媒体栈都能接**：已适配 **Plex**（[Tautulli](/usage/tautulli)、[官方 Webhooks](/usage/plex-webhooks)）、**[Emby](/usage/emby)**、**[Jellyfin](/usage/jellyfin)** 与 **[自定义 Webhook](/usage/custom-webhook)**。另可按需启用 **[Trakt.tv 定时同步](/usage/trakt)**、**[飞牛影视](/usage/feiniu)** 、**[fongmi](/usage/fongmi)**等接入方式，覆盖了绝大多数场景。
- 🔄 **Trakt.tv 同步**：支持按计划从 Trakt 拉取观看记录并写回 Bangumi，便于在 Trakt 生态里补全或校正进度。
- 🗄️ **飞牛同步**：只读读取飞牛上的观看进度，达到「已看完」或设定进度阈值后同步到 Bangumi，默认关闭。
- 🗄️ **fongmi**：读取fongmi app上的观看进度，达到「已看完」或设定进度阈值后同步到 Bangumi，默认关闭。
