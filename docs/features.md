...
- 🧩 **匹配记录可追溯**：每条同步记录都会保留完整的匹配过程（自定义映射 → bangumi-data → API 搜索三段式），在「匹配记录」页面可按匹配方式、平台筛选查看，定位「为什么没对上」更直观；支持在页面内直接发起测试匹配，实时验证标题归一化与平台加权排序效果。
- 🧰 **低维护成本**：启动时自动清理超期同步记录，控制 SQLite 数据库体积（默认不清理，可按需设置 30–90 天）；列表查询已剔除大体积 JSON 字段，分页加载更快；`config.ini` 模板化并支持首次运行自动复制，部署升级更顺滑。
