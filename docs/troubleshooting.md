...
程序使用本地 SQLite 文件（`sync_records.db`）保存同步与匹配记录，长期运行后体积会逐渐增大，通常与以下因素有关：

### 6.1 保留天数设置过大或未开启清理

`config.ini` 的 `[dev]` 段 `sync_records_retention_days` 控制启动时自动清理超过指定天数的记录：

- `0` 或负数（默认）：**永不清理**，数据库会持续增长（仅适合需要完整审计的场景）
- `30`：保留最近 30 天，适合大多数生产场景
- 调试环境可设为 `7`–`14`，生产环境建议 `30`–`90`

排查方法：检查当前配置值，确认不是 `0`；启动日志里会输出 `已清理 N 条超过 X 天的同步记录`，若从未见过该日志且数据库体积较大，说明清理未生效。

### 6.2 匹配记录 JSON 体积较大

每条同步记录的 `match_trace` 字段会保存完整的三段式匹配过程（含候选列表），单条记录可能达到数 KB。该字段仅在**详情查询**返回，列表查询已默认剔除以加快分页加载，但写入时仍会完整存储。

排查方法：若数据库体积明显异常（如几百 MB），可考虑临时调小 `sync_records_retention_days` 并重启程序触发清理。

### 6.3 手动清理历史数据

如需立即清理而不等待下次启动，可直接操作数据库文件：

1. 停止程序（避免 SQLite 锁冲突）
2. 备份 `sync_records.db`
3. 执行 `DELETE FROM sync_records WHERE timestamp < datetime('now', '-30 days')`
4. 执行 `VACUUM;` 回收空间
5. 重启程序

::: warning 注意
直接操作数据库前务必备份；清理后仪表板热力图缓存会自动失效并重新加载。
:::
