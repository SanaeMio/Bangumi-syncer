...
| 某个 Bangumi 账号一直不同步                    | [1. Bangumi 账号问题](#_1-bangumi-账号问题)                           |
...
- **账号已停用**：在「Bangumi 账号配置」中被停用的账号不参与任务同步，条目仍保留在列表中，点「启用」即可恢复。
...
1. **检查 Bangumi 账号配置**：探测使用当前首选的 Bangumi 账号，若账号未配置或访问令牌为空会直接返回失败（日志：`📚 无可用账号配置用于探测 API`）。可在「配置管理」→「Bangumi 账号配置」中通过 OAuth 授权或手动添加账号。
