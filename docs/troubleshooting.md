...
| 多用户模式下「找不到对应 Bangumi 账号」        | [4. 未设置媒体服务器用户名](#_4-未设置媒体服务器用户名)               |
...
- **未配置 Bangumi 账号**：在「配置管理」里填写 Bangumi 的用户名和访问令牌，保存后重试。
- **access_token 失效**：长期运行后 token 可能过期（Bangumi 的 token 有效期约 1 年），到「配置管理」里重新 [生成并填写](https://next.bgm.tv/demo/access-token) 一次。
...
1. **检查 Bangumi 账号配置**：探测使用的账号来自 `[sync] mode` 对应的段：
   - `mode = single`（默认）→ 读 `[bangumi]` 段的 `用户名` / `访问令牌`。
