...
- **更快**：本地精确查询 `0.03ms`（基于 SQLite FTS5 trigram 索引），原本要走 API 的查询延迟从秒级降到毫秒级。
...
| 磁盘空间紧张（可用 <3GB） | ⚠️ 不建议     | active 库 0.8GB + 解压 1GB + 新库 0.8GB + WAL 余量 ≈ **2.6GB**（zip 已删） ，详见下文「磁盘占用」 |
| 排查匹配问题              | ⚠️ 可临时关闭 | 关闭后走纯 API 路径，便于对比是 archive 数据问题还是 API 数据问题                                 |
| 调试环境                  | ❌ 建议关闭   | 测试框架默认强制禁用，避免 FTS5 表构建阻塞测试                                                    |
...
| `data_dir`          | `./data/archive` | 数据目录，存放双库 `bangumi_archive_a.db` / `bangumi_archive_b.db`（含内嵌 FTS5 标题索引）、`bangumi_archive.active` 指针、`bangumi_archive.meta` 元数据 |
...
::: tip 配置立即生效
保存配置后会自动调用 `bangumi_archive.reload_config()` 与 `bangumi_archive_scheduler.apply_config_after_save()`，**无需重启程序**。从「关闭」切到「开启」时会自动触发首次导入（FTS5 标题索引在导入时一并构建）。
:::
...
2. **后台导入**：从 GitHub 下载 dump zip（约 400MB，含 SHA256 校验），导入到 SQLite 双库中的 inactive 库，导入时同步构建 FTS5 trigram 标题索引，导入成功后切换 active 指针并清空旧库
3. **查询就绪**：导入完成后 FTS5 表随 db 一并可用，无需额外后台构建，**不阻塞主流程**
4. 导入完成前 archive 查询会返回 `archive_miss` 自动降级到 API，调用方无感知
...
5. **剥离书名号/方括号包裹后再精确匹配**
6. **模糊兜底**（FTS5 trigram OR 预筛 + rapidfuzz 多 scorer 融合）
...
双库**仅在导入时短暂并存**（active 库 + 新库），导入完成切换指针后旧库立即清理，常态仅保留 active 库。

| 项目                                                | 大小         |
| --------------------------------------------------- | ------------ |
| active 库（导入前已存在，含 FTS5 标题索引）         | 约 0.8GB     |
| 新库（导入时新建，写入非 active 库）                | 约 0.8GB     |
| 解压后的 JSON Lines（解压完成立即删除 zip，已不计） | 约 1GB       |
| WAL 余量                                            | < 0.1GB      |
| **导入峰值合计**                                    | **约 2.6GB** |
| **常态占用**（仅 active 库）                        | **约 1.3GB** |

导入成功切换 active 指针后，旧库（db / db-wal / db-shm）会自动清理。

> 导入时仅保留 `type∈(2,6)`（动画 + 三次元）的 subject，并移除 `subject_character` /
> `subject_person` / `person_relation` 三张运行时未使用的死表，单库体积从 ~1.4GB 降至 ~0.8GB。

`min_disk_space_mb = 3000` 是**导入前**的硬性阈值，低于此值会跳过导入并记录错误日志。下载/上传/解压/导入均在 `data_dir/.tmp/` 完成；解压后立即删除 zip，导入峰值 = active 库 ~0.8GB + 解压 JSON Lines ~1GB + 新库 ~0.8GB + WAL 余量 ≈ 2.6GB，阈值预留约 400MB 余量。

## 内存占用

标题索引基于 SQLite **FTS5 trigram** 虚拟表（contentless 模式），随 db 文件落盘，**进程内存占用接近 0**（仅 SQLite 缓存页）。
...
```
data/archive/
├── bangumi_archive_a.db          # 双库 a（含 subject_fts 虚拟表）
├── bangumi_archive_b.db          # 双库 b（含 subject_fts 虚拟表）
...
| 操作                                                  | 耗时                     |
| ----------------------------------------------------- | ------------------------ |
| 精确标题查询（FTS5 phrase MATCH + 关联过滤）          | `0.03ms`                 |
| 模糊标题查询（FTS5 trigram OR 预筛 + rapidfuzz 精排） | `0.08ms`（原 50-300ms）  |
| FTS5 表构建（导入时一次性）                           | 约 `7s`                  |
| 进程内存占用（标题索引部分）                          | 接近 `0`（原 200-350MB） |

2 字符查询（占比 <1%）回退 subject 表 LIKE，约 `200ms`。
...
archive 启用但 FTS5 表尚未随导入完成时，`try_search` / `try_search_old` 会返回 `archive_miss` 自动降级到 API，**不影响同步成功率**，只是查询走 API 较慢。其他 `try_get_*` 方法（直接查 SQLite 不依赖 FTS5 索引）可正常工作。
