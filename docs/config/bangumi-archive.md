...
| **BK-tree 模糊匹配** | 关闭          | 开启后对归档标题构建 BK-tree 索引，支持编辑距离模糊查询，提升形近/缺字标题的召回率。首次启用在后台异步构建，不阻塞查询 |
| **导入失败重试间隔**（`retry_interval`） | `3600` | 导入失败后间隔多少秒再重试。此项没有配置页入口，需要时手动改 `config.ini` 后重启 |
