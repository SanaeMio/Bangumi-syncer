...
API 不可达（TTL 内）时，**匹配新条目的读链路本身已短路**（archive 未启用时搜索必失败）。为避免每条同步任务反复走完完整匹配流程、反复产出「未找到匹配的番剧」error 记录：

- 匹配入口检测到 API 不可达 + Replay 开启时，**本轮直接跳过匹配**，不写 error 记录，并尝试提前唤醒一次补发探测（500ms 防抖）
- 恢复统一由调度器的**批量探测**驱动：探测成功一次，即一次性复位所有用户缓存实例的不可达标记，各用户实例无需再等各自 TTL 到期后逐条重试
- 未开启 Replay 时保持原行为（不可达也照常走匹配流程）
