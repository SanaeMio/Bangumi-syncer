...
- 在「待确认」页**拒绝**某条候选时，相关标题会自动加入此列表（标签上会标注「拒绝时记录」）。

::: tip 自定义映射优先
如果某个番剧既命中了屏蔽关键词、又设置了**自定义映射**，程序会按自定义映射同步——自定义映射代表你明确指定「我就要同步这一部」，优先级高于屏蔽规则。
:::

::: info 关于存储位置
屏蔽关键词保存在数据库中，通过网页界面管理，**不在配置文件里**。
如果你的旧配置 `config.ini` 的 `[sync]` 段还写着 `blocked_keywords`，程序首次启动时会自动把这些关键词导入数据库（仅导入一次，之后以网页界面为准）。
:::
