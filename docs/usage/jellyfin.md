...
4. 展开下方的 `Generic`，`Webhook Name` 随便填，`Webhook Url` 输入 `http://{ip}:8000/Jellyfin`，`ip` 根据本机情况填写。`Notification Type` 只选中 `Playback Stop`，`Item Type` 只选中 `Episodes`。`Template` 填写如下模版，然后点击 `Save` 保存设置：
