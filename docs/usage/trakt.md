...
::: tip 授权成功后会自动补充用户名
授权成功后，系统会自动把你登录本程序的用户名追加到**当前首选的 Bangumi 账号**的「媒体服务器用户名」列表里（如果尚未存在），并在成功页弹出提示。这是为了让 Trakt 同步的观看记录能正确路由到对应的 Bangumi 账号，避免被用户名过滤拦截。如果你不希望如此，可在「Bangumi 账号配置」里手动移除该用户名。
:::
...
::: tip 自动补充用户名
邮箱登录成功后，同样会自动把你登录本程序的用户名追加到首选的 Bangumi
